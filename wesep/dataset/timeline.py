...
def two_speaker_crossing(overlap_ratio, rng):
    # One speaker leads and the other trails with the requested overlap.
    overlap_start = rng.uniform(0.0, 1.0 - overlap_ratio)
    overlap_end = overlap_start + overlap_ratio

    if rng.random() < 0.5:
        a_start, a_end = 0.0, overlap_end
        b_start, b_end = overlap_start, 1.0
    else:
        b_start, b_end = 0.0, overlap_end
        a_start, a_end = overlap_start, 1.0
