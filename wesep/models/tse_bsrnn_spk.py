...
class TSE_BSRNN_SPK(nn.Module):
    ...
    def __init__(self, config, defer_pretrained=False):
        ...
        channels = sep_configs["channels"]
        for feature in ("listen", "usef"):
            if (channels > 1
                    and self.spk_configs["features"][feature]["enabled"]):
                raise ValueError(
                    f"{feature} currently supports single-channel mixtures "
                    "only")
        self.spk_configs["features"]["usef"]["spec_dim"] = 2 * channels
        ...
        if self.spk_configs["features"]["usef"]["enabled"]:
            ...
            sep_configs["spec_dim"] = 2 * channels
        if self.spk_configs["features"]["tfmap"]["enabled"]:
            sep_configs["spec_dim"] += 1
    ...
    def forward(self, batch):
        ...
        if self.spk_configs['features']['listen']['enabled']:
            ...
            wav_mix = self.spk_ft.listen.compute(
                enroll, wav_mix.squeeze(1)).unsqueeze(1)
        ###########################################################
        ...
        reference_spec = spec[:, self.sep_model.reference_channel]
        ###########################################################
        # C1. Feature: usef
        if self.spk_configs['features']['usef']['enabled']:
            # C1.1 Generate the USEF feature
            enroll_spec = self.sep_model.stft(enroll)[-1]
        ...
        subband_mix_spec = self.sep_model.band_split(
            reference_spec)  # list of (B, BW, T) complex
        ...
        if self.spk_configs['features']['spkemb']['enabled']:
            ...
            enroll_emb = self.spk_ft.spkemb.compute(enroll)  # (B, F_e)
        ...
        s = self.sep_model.istft(est_complex, length=wav_mix.shape[-1])
        ###########################################################
        # C0. Feature: listen
        if self.spk_configs['features']['listen']['enabled']:
            # C0.2 Prepend the enroll to the mix in the beginning
            s = self.spk_ft.listen.post(s, mix_len=mix_len)  # (B, S, T)
        ###########################################################
        return {"speech": s}
...
def check_causal(model):
    ...
    with torch.no_grad():
        out1 = model({
            "wav_mix": input.unsqueeze(1),
            "audio_aux": enroll.unsqueeze(1),
        })["speech"]
        for i in range(fs * 1, fs * 4, fs):
            inputs2 = input.clone()
            inputs2[..., i:] = 1 + torch.rand_like(inputs2[..., i:])
            out2 = model({
                "wav_mix": inputs2.unsqueeze(1),
                "audio_aux": enroll.unsqueeze(1),
            })["speech"]
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        output = model({
            "wav_mix": mix.unsqueeze(1),
            "audio_aux": enroll.unsqueeze(1),
        })
    print(output["speech"].shape)
