...
class UsefFeature(nn.Module):
    """Encode and combine enrollment and mixture spectra using USEF-TSE."""

    def __init__(self, config, eps=torch.finfo(torch.float32).eps):
        super().__init__()
        self.causal = config["causal"]
        norm_type = "LN" if self.causal else "GN"
        kernel_size = (config["t_ksize"], 3)
        padding = (0, 1) if self.causal else (config["t_ksize"] // 2, 1)
        self.left_context = config["t_ksize"] - 1
        ...
        if not self.causal:
            ...
            self.usef_con2v = nn.Sequential(
                select_norm(
                    norm_type,
                    (config["spec_dim"], config["enc_dim"]),
                    eps,
                ),
                nn.Conv2d(
                    config["spec_dim"],
    ...
    def compute(self, enroll, mix):
        """Return enrollment and mixture features from RI spectra ``(B,2,F,T)``."""
        mix = mix.permute(0, 1, 3, 2).contiguous()
        enroll = enroll.permute(0, 1, 3, 2).contiguous()

        # Encode both spectra with the shared causal or non-causal encoder.
        if not self.causal:
            mix = self.usef_con2v(mix)
            enroll = self.usef_con2v(enroll)
        else:
            mix = mix.permute(0, 2, 1, 3).contiguous()
            mix = self.usef_con2v[0](mix)
            mix = mix.permute(0, 2, 1, 3).contiguous()
            mix = F.pad(mix, (0, 0, self.left_context, 0))
            mix = self.usef_con2v[1](mix)

            enroll = enroll.permute(0, 2, 1, 3).contiguous()
            enroll = self.usef_con2v[0](enroll)
            enroll = enroll.permute(0, 2, 1, 3).contiguous()
            enroll = F.pad(enroll, (0, 0, self.left_context, 0))
            enroll = self.usef_con2v[1](enroll)

        enroll = self.usef_att(mix, enroll)
        enroll = enroll.permute(0, 1, 3, 2).contiguous()
        mix = mix.permute(0, 1, 3, 2).contiguous()
        return enroll, mix

    def post(self, mix_repr, feat_repr):
        """Concatenate USEF enrollment and mixture representations."""
