...
mix_data_path=$(realpath "${mix_data_path}")
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
  ...
  # for dset in train-360; do
    ...
    python "${WESEP_ROOT}/tools/build_speaker_labels.py" \
      --samples "${mix_index}" \
      --output "${real_data}/${noise_type}/${dset}/spk2label.json"
...
labels:
  speaker_label:
    resource: ${data}/${noise_type}/${dset}/spk2label.json
EOF
  done
fi
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  if [ ! -f wespeaker_models/voxceleb_resnet34/avg_model.pt ]; then
    wget -O wespeaker_models/voxceleb_resnet34.zip \
      https://wespeaker-1256283475.cos.ap-shanghai.myqcloud.com/models/voxceleb/voxceleb_resnet34.zip
    unzip -o wespeaker_models/voxceleb_resnet34.zip -d wespeaker_models
  fi
  if [ ! -f wespeaker_models/voxceleb_ECAPA512/avg_model.pt ]; then
    wget -O wespeaker_models/voxceleb_ECAPA512.zip \
      https://wespeaker-1256283475.cos.ap-shanghai.myqcloud.com/models/voxceleb/voxceleb_ECAPA512.zip
    unzip -o wespeaker_models/voxceleb_ECAPA512.zip -d wespeaker_models
  fi
fi
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
  ...
  python "${WESEP_ROOT}/tools/make_lmdb.py" ${real_data}/musan/wav.scp ${real_data}/musan/lmdb
fi
