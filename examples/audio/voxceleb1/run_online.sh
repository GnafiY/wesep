...
# Use launcher GPUs when provided; otherwise read the recipe default.
if [ -z "${gpus}" ]; then
  gpus=$(python -c 'import json, sys, yaml; c = yaml.safe_load(open(sys.argv[1])); print(json.dumps(c.get("gpus", [0]), separators=(",", ":")))' "${config}")
fi
gpus=${gpus:-"[0]"}
gpus=${gpus// /}
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
  ...
  for dset in train-vox1; do
    python "${WESEP_ROOT}/tools/make_shards_from_samples_online.py" \
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
  ...
  torchrun --standalone --nnodes=1 --nproc_per_node=$num_gpus \
    "${WESEP_ROOT}/wesep/bin/train.py" --config $config \
    --exp_dir ${exp_dir} \
    --gpus "${gpus}" \
    --num_avg ${num_avg} \
    --data_type "${data_type}" \
